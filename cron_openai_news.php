...
// ✅ Function: Fetch News from OpenAI
function getNewsFromOpenAI($api_url, $api_key, $category) {
    $postData = [
        "model" => "gpt-4",
        "messages" => [
...
foreach ($categories as $category) {
    $openai_response = getNewsFromOpenAI($api_url, $api_key, $category);

    if ($openai_response) {
        $articles = preg_split("/\n\s*\n/", trim($openai_response));
        $inserted = 0;
        $skipped = 0;
        ...
        foreach ($articles as $article) {
            $lines = explode("\n", trim($article));
            $title = trim($lines[0]);
            $title = trim($title); // Clean extra spaces
